...
Bom, até agora já sabemos o que o R é e também já sabemos utilizar o R como uma calculadora. Vamos agora aprender a "guardar" os nossos dados. Isso é relativamente muito simples. Basta associar uma determinada palavra com um valor. Por exemplo,

```{r}
x = 1
```

Acabeid de falar pro R guardar no "x" o valor 1. Podemos fazer isso com palavras
...
Repare que dessa vez eu não utilizei o sinal de igual (=). No R, podemos utilizar essa seta (<-) como equivalente do igual (=). De maneira geral, utilizamos a **<-** quando atribuímos um valor a uma variável. Deixamos o "=" para outras situações que veremos mais a frente.
...
Lembra quando testasmos se uma afirmação é verdadeira? O resultado dessas operações são as nossas variáveis booleanas. Como ela testa a 
